        ...
        for (let attempt = 1; attempt <= MAX_UPLOAD_RETRIES; attempt++) {
            ...
            if (!urls) {
                if (attempt < MAX_UPLOAD_RETRIES) {
                    logger.warn(`Media upload failed on all hosts, retrying (${attempt}/${MAX_UPLOAD_RETRIES})...`);
                    await new Promise(r => setTimeout(r, 2000));
                    continue;
                }
                logger.error(`Media upload failed on all hosts after ${MAX_UPLOAD_RETRIES} attempts`);
                return null;
            }
            return urls;
        }
