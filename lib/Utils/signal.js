...
export const generateOrGetPreKeys = async (creds, range) => {
    ...
    if (remaining > 0) {
        for (let i = creds.nextPreKeyId; i <= lastPreKeyId; i++) {
            ...
            if ((i - creds.nextPreKeyId + 1) % 50 === 0) {
...
export const getNextPreKeys = async ({ creds, keys }, count) => {
    const { newPreKeys, lastPreKeyId, preKeysRange } = await generateOrGetPreKeys(creds, count);
    const update = {
