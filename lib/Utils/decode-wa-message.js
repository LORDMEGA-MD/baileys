...
// Outgoing @bot message id → messageSecret. Populated when we send to @bot (or see our
// own echo), consumed when the msmsg response from @bot arrives and needs decrypting.
...
export function decodeMessageNode(stanza, meId, meLid) {
    ...
    if (isPnUser(from) || isLidUser(from) || isHostedLidUser(from) || isHostedPnUser(from)) {
        ...
        // @bot responses (Meta AI) arrive from a bare @bot jid — treat them as a 1:1 chat
        msgType = 'chat';
        chatId = from;
        author = from;
        fromMe = false;
    ...
    return {
        ...
        async decrypt() {
            ...
            if (Array.isArray(stanza.content)) {
                // msmsg replies carry their key material in sibling <meta>/<bot> nodes, so
                // collect those before walking the enc nodes.
                let metaTargetId = null;
                let metaTargetSenderJid = null;
                let botEditTargetId = null;
                let botType = null;
                if (stanza.content.some(n => n?.attrs?.type === 'msmsg')) {
                    for (const { tag, attrs } of stanza.content) {
                        ...
                        if (tag === 'bot' && attrs && 'edit_target_id' in attrs) {
                            botEditTargetId = attrs.edit_target_id; // can be '' for the 'first' type
                        }
                    ...
                    try {
                        ...
                        switch (e2eType) {
                            ...
                            case 'msg':
                                ...
                                break;
                            case 'msmsg': {
                                // botType: null = non-streaming (no <bot> node), 'full'/'last' = complete
                                // response, 'first' = streaming partial, which we intentionally skip.
                                ...
                                for (const item of botRecentSecretsByChat.get(sender) || []) {
                        ...
                        if (!msgBuffer) {
                            // a skipped streaming msmsg partial leaves nothing to decode
                            continue;
                        }
                        ...
                        // deviceSentMessage.message may omit messageContextInfo (which carries the
                        // messageSecret for @bot) even though the outer wrapper has it — preserve it
                        ...
                        // stash the messageSecret from any message — the AI group server can set it
                        // on any of them, and the msmsg reply needs it to decrypt
                        const secret = msg.messageContextInfo?.messageSecret;
                        if (secret) {
                            setBotMessageSecret(fullMessage.key.id, secret, fullMessage.key.remoteJid);
                        }
                    }
                    catch (err) {
                        const errorContext = {
