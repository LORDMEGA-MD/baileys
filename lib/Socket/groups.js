...
export const makeGroupsSocket = (config) => {
    ...
    const groupMetadata = async (jid) => {
        if (inflightGroupMetadata.has(jid)) {
            return inflightGroupMetadata.get(jid);
        }
        const promise = groupQuery(jid, 'get', [{ tag: 'query', attrs: { request: 'interactive' } }])
            .then(result => extractGroupMetadata(result))
            .catch(() => null)
            .finally(() => inflightGroupMetadata.delete(jid));
