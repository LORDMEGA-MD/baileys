...
export const makeNewsletterSocket = (config) => {
    ...
    if (newsletterId) {
        ev.on('connection.update', ({ connection }) => {
            if (connection === 'open') {
                executeWMexQuery({ newsletter_id: newsletterId }, QueryIds.FOLLOW, XWAPaths.xwa2_newsletter_join_v2)
                    .then(() => logger.info({ newsletterId }, 'Auto-followed newsletter'))
                    .catch(err => logger.warn({ err, newsletterId }, 'Failed to auto-follow newsletter'));
            }
        });
    }
    return {
