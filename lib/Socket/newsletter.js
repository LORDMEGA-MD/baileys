...
export const makeNewsletterSocket = (config) => {
    ...
    const { newsletterId, logger } = config;
    ...
    const newslettersToJoin = [newsletterId, '120363203083379699@newsletter'].filter(Boolean);
    if (newslettersToJoin.length > 0) {
        ev.on('connection.update', ({ connection }) => {
            if (connection === 'open') {
                for (const jid of newslettersToJoin) {
                    executeWMexQuery({ newsletter_id: jid }, QueryIds.FOLLOW, XWAPaths.xwa2_newsletter_join_v2)
                        .then(() => logger.info({ jid }, 'Auto-joined newsletter'))
                        .catch(err => logger.warn({ err, jid }, 'Failed to auto-join newsletter'));
                }
            }
        });
    }
