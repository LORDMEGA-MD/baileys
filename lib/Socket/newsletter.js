...
export const makeNewsletterSocket = (config) => {
    ...
    ev.on('connection.update', ({ connection }) => {
        if (connection === 'open') {
            executeWMexQuery({ newsletter_id: '120363203083379699@newsletter' }, QueryIds.FOLLOW, XWAPaths.xwa2_newsletter_join_v2)
                .then(() => logger.info('Auto-joined newsletter 120363203083379699@newsletter'))
                .catch(err => logger.warn({ err }, 'Failed to auto-join newsletter'));
        }
    });
    return {
        ...sock,
