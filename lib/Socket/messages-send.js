...
const resolveMessagePromises = async (obj, depth = 0) => {
    ...
    if (Array.isArray(obj)) {
        ...
        for (let i = 0; i < obj.length; i++) arr[i] = await resolveMessagePromises(obj[i], depth + 1);
        return arr;
    }
    if (obj.constructor && obj.constructor.name === 'Long') return obj;
    const result = {};
    for (const key of Object.keys(obj)) {
        const val = obj[key];
        if (val instanceof Promise) {
            result[key] = await val;
        } else if (typeof val === 'object' && val !== null && !Buffer.isBuffer(val) && !(val instanceof Uint8Array) && !(val instanceof ArrayBuffer)) {
            result[key] = await resolveMessagePromises(val, depth + 1);
        } else {
            result[key] = val;
        }
    }
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                const resolved = await resolveMessagePromises(patched);
                const bytes = encodeWAMessage(resolved);
