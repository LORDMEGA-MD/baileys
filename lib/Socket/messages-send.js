...
import { setBotMessageSecret } from '../Utils/decode-wa-message.js';
...
export const makeMessagesSocket = (config) => {
        ...
        return mediaConnMutex.mutex('media-conn', async () => {
            ...
            if (!media || forceGet || new Date().getTime() - media.fetchDate.getTime() > media.ttl * 1000) {
                const pending = (async () => {
                    ...
                    if (!mediaConnNode) {
                        throw new Boom('Media connection node not found', { statusCode: 500 });
                    }
                    const node = {
                ...
                mediaConn = pending;
                // A failed fetch must not stay cached: the `await mediaConn` above would rethrow
                // the same rejection on every later call, so one transient server hiccup would
                // break media for the rest of the socket's life. Guard against clobbering a newer
        ...
        if (jidsRequiringFetch.length) {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                try {
                    ...
                    lastError = err;
                    if (err?.data === 503 && attempt < 2) {
                        logger.warn({ attempt, jids: jidsRequiringFetch.length }, 'service-unavailable fetching sessions, retrying');
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList, statusPrivacy }) => {
        ...
        // status audience privacy: 'contacts' | 'allowlist' | 'denylist'
        ...
        if (isGroupOrStatus && !isRetryResend) {
            const [groupData, skm] = await Promise.all([
                (async () => {
        ...
        await sendNode(stanza);
        // Register the messageSecret at send time so an msmsg reply can be decrypted even if
        // it arrives before our own message echo comes back.
        ...
        // Phase 2: Only key storage mutations inside transaction
        await authState.keys.transaction(async () => {
            if (isGroupOrStatus && !isRetryResend && senderKeyMap) {
                await authState.keys.set({ 'sender-key-memory': { [jid]: senderKeyMap } });
            }
            if (clearedTokenEntry) {
                try {
                    await authState.keys.set({ tctoken: { [tcTokenJid]: clearedTokenEntry } });
                }
                catch (err) {
        ...
        return msgId;
            ...
            logger.error({ jid, msgId, err: err?.message }, 'relayMessage failed');
            return msgId || null;
        }
    };
