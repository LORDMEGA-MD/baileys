...
export const makeMessagesRecvSocket = (config) => {
    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            const isConnClosed = /connection closed|connection terminated|connection was lost|WebSocket Error/i.test(error?.message || error?.output?.payload?.error || '');
            if (isConnClosed) {
                logger.debug({ error: error?.message }, 'connection closed while handling message');
            } else {
                logger.error({ error, node: binaryNodeToString(node) }, 'error in handling message');
            }
            if (!acked && !isConnClosed) {
