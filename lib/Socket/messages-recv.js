...
export const makeMessagesRecvSocket = (config) => {
    ...
    /**
     * Handles the <status> stanzas the server pushes for newsletter posts. Arrives on the
     * CB:status channel, AB-gated by status_e2ee_recv_over_status_stanza.
     */
    const handleNewsletterStatus = async (node) => {
        ...
        // optional <meta> child: edit timestamps, interaction type, parent/response ids
        ...
        const meta = metaNode
            ...
            : undefined;
        const viewsNode = getBinaryNodeChild(node, 'views_count');
        const viewsCount = viewsNode?.attrs?.count !== undefined ? +viewsNode.attrs.count : undefined;
        const responsesNode = getBinaryNodeChild(node, 'responses_count');
        ...
        const reactionCounts = getBinaryNodeChildren(reactionsNode ?? { content: [] }, 'reaction').map(r => ({
            code: r.attrs.code,
            count: +r.attrs.count
        }));
        let content = null;
        ...
        if (type === 'reaction') {
            content = { type: 'reaction', code: getBinaryNodeChild(node, 'reaction')?.attrs?.code };
        }
            ...
            if (edit === '7' || edit === '8') {
                ...
                try {
                    const buf = Buffer.isBuffer(plaintextNode.content)
                        ...
                        : Buffer.from(plaintextNode.content);
                    const message = proto.Message.decode(buf).toJSON();
    ...
    const handleMessage = async (node) => {
        ...
        // msmsg (Meta AI bot reply) needs the messageSecret of the message we sent to @bot.
        // The in-memory map is lost on restart, so recover it from the message store.
        if (encNode?.attrs.type === 'msmsg' && getMessage) {
            const targetId = getBinaryNodeChild(node, 'meta')?.attrs?.target_id;
            if (targetId) {
                try {
                    ...
                    if (secret) {
                        setBotMessageSecret(targetId, secret, node.attrs.from);
                    }
                }
                catch (err) {
        ...
        try {
            ...
            if (!!alt) {
                ...
                // Server sent none of participant_pn/sender_pn/peer_recipient_pn — happens for
                // senders who have a WhatsApp username set — so there's no alt JID in the stanza.
                // Recover it from the local LID->PN store, which outgoing sends and USync keep
                // populated, so consumers keyed by phone number still resolve the sender.
                const primaryJid = msg.key.participant || msg.key.remoteJid;
                if (isLidUser(primaryJid)) {
                    try {
                        // getPNForLID returns a device-scoped JID; stanza attrs carry it without
                        // the device, so normalize to match. jidNormalizedUser yields '' for an
                        // undecodable JID, so guard before assigning.
                        ...
                        logger.debug({ err, primaryJid }, 'failed to recover PN for LID-addressed sender');
                    }
                }
            }
            await messageMutex.mutex(async () => {
                ...
                const maxAge = config.messageHistoryMaxAgeSeconds;
                if (maxAge > 0 && msg.messageTimestamp) {
                    const msgAge = unixTimestampSeconds() - toNumber(msg.messageTimestamp);
                    if (msgAge > maxAge) {
                        logger.debug({ msgId: msg.key.id, msgAge, maxAge }, 'skipping old message');
                        acked = true;
    ...
    const handleCall = async (node) => {
        try {
            ...
            if (status === 'offer') {
                ...
                const audioNodes = getBinaryNodeChildren(infoChild, 'audio');
                const silenceNode = getBinaryNodeChild(infoChild, 'silence');
                ...
                // wire attrs are 'enc'/'rate' for audio and 'dec' for video, not 'codec';
                // <audio> repeats once per supported sample rate
                ...
                // the raw SRTP key arrives Signal-encrypted in an <enc> child
                ...
                if (encNode?.content) {
                    try {
                        ...
                        const decrypted = await signalRepository.decryptMessage({
                            jid: attrs.from,
                            type: encNode.attrs.type || 'msg',
                            ciphertext
                        });
            ...
            enrichCallSignalling(call, infoChild, status);
            if (isCallEndedStatus(status)) {
    ...
    /** Tags that carry actual call state; the rest are media/relay signalling we only ack. */
    ...
    const isCallEndedStatus = (status) => status === 'reject' ||
        status === 'accept' ||
        status === 'timeout' ||
        status === 'terminate' ||
    ...
    const parseGroupCallRoster = (groupInfoNode) => getBinaryNodeChildren(groupInfoNode, 'user').map(userNode => ({
        jid: userNode.attrs.jid,
        state: userNode.attrs.state,
        userPn: userNode.attrs.user_pn,
        devices: getBinaryNodeChildren(userNode, 'device').map(d => ({
            jid: d.attrs.jid,
            platform: d.attrs.platform
        }))
    }));
    /** Fills in the signalling-only fields, given the node the state attrs live on. */
    const enrichCallSignalling = (call, stateNode, status) => {
        if (status === 'peer_state') {
            call.state = getBinaryNodeChild(stateNode, 'peer_state')?.attrs?.state || stateNode.attrs?.state;
        }
        else if (status === 'group_info') {
            call.payload = stateNode.attrs;
            call.participants = parseGroupCallRoster(stateNode);
        }
        else if (status === 'video_state') {
            const enabledRaw = getBinaryNodeChild(stateNode, 'video_state')?.attrs?.enabled ?? stateNode.attrs?.enabled;
            call.enabled = enabledRaw === 'true' || enabledRaw === '1';
        }
        else if (status === 'mute') {
            // mute_v2 IS the tag itself on real traffic — attrs are
            // { call-id, call-creator, mute-state: '0'|'1' }, with no nested <audio> child
            call.muted = stateNode.attrs['mute-state'] === '1';
        }
    };
    /**
     * Some call signalling arrives as a top-level stanza (<offer>, <terminate>, …) instead of
     * being wrapped in <call>, so it needs its own handler — state attrs live on the node itself.
     */
    const handleStandaloneCallStanza = async (node) => {
        try {
            if (!CALL_STATE_TAGS.has(node.tag)) {
                return; // media/relay signalling (transport, video, duration, lobby, …): ack only
            }
            const { attrs } = node;
            ...
            if (status === 'offer') {
                const videoNode = getBinaryNodeChild(node, 'video');
                const audioNodes = getBinaryNodeChildren(node, 'audio');
                const silenceNode = getBinaryNodeChild(node, 'silence');
                call.isVideo = !!videoNode;
                ...
                call.peerJid = attrs.from || attrs['peer-jid'] || attrs['user-jid'];
            }
            ...
            enrichCallSignalling(call, node, status);
            if (callId && isCallEndedStatus(status)) {
            ...
            await sendMessageAck(node).catch(ackErr => logger.error({ ackErr }, 'failed to ack standalone call stanza'));
        }
    };
    ...
    // Call signalling that arrives top-level instead of wrapped in <call>. Tags not in
    // CALL_STATE_TAGS are acked only — without a handler the server redelivers them forever.
