    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            if (!!alt) {
                ...
                // Senders with a WhatsApp username arrive LID-addressed without any *_pn
                // attr, so senderAlt is empty. Backfill the PN from the mapping store so
                // consumers keyed by phone number still resolve the sender.
                const primaryJid = msg.key.participant || msg.key.remoteJid;
